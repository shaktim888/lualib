...
typedef void(^ Bool_CallBack)(BOOL success);

@implementation AppDelegate


void myAssert(bool val, NSString * s) {
    if(!val ) {
        NSLog(@"Assert failure:%@", s);
    }
}

typedef int(^back)(void);

- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    
    char * p2 = @encode(back);
    
    NSString *filepath = [[NSBundle mainBundle] pathForResource:@"script/check.oc" ofType:nil];
    NSData* data = [NSData dataWithContentsOfFile:filepath];
    NSString* string = [[NSString alloc] initWithBytes:[data bytes]
                                                length:[data length]
                                              encoding:NSUTF8StringEncoding];
    ASTNode* root = [ASTUtil parseString:string];
...
//
//    NSString * text = @"\
//    NSString *numberOfRowsInSection = @\"\
//    NSString * dd = @\\\"data\\\";\
//    NSArray *array = objc_getAssociatedObject(self, dd);\
//    if (array != nil) {\
//    return array.count;\
//    }else{\
//    return 0;\
...
//    [OCCfuntionHelper defineCFunction:@\"NSLog\" types:@\"void,NSString *,NSString *\"];\
//    [OCCfuntionHelper defineCFunction:@\"class_getMethodImplementation\" types:@\"IMP,Class,SEL\"];\
//    [OCCfuntionHelper defineCFunction:@\"class_addMethod\" types:@\"BOOL,Class,SEL,IMP,char *\"];\
//    NSString *numberOfRowsInSection = @\"\
//    NSString * dd = @\\\"data\\\";\
