...
+ (void)pushGlobalValue:(id)value forKey:(NSString *)key {
    if(!globalCtx) {
        globalCtx = [[NSMutableDictionary alloc] init];
    }
    [globalCtx setValue:value forKey:key];
}

- (void)setToGlobal:(NSString *)key {
    ASTVariable* val = [self fetchValueFromKey:key];
    if(val.value) {
        [globalCtx setValue:val forKey:key];
    }
}
