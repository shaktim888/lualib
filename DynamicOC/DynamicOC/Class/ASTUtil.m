...
+ (ASTNode*)string_node:(NSString*)value
{
    ...
    for( int i = 0 ; i < value.length; i++) {
        NSString *character = [value substringWithRange:NSMakeRange(i, 1)];
        if([character isEqualToString:@"\\"]) {
            i++;
            if(i < value.length) {
                character = [value substringWithRange:NSMakeRange(i, 1)];
...
+ (ASTType*)typeForNodeType:(ASTNodeType)type keyword:(NSString*)keyword
{
    if(!_typeEncodeDict)
    {
        ...
        {
            ASTType* ast_ = [[ASTType alloc]init];
            ast_.type = ASTNodeTypeBlockDeclarator;
            ast_.keyword = @"block";
            ast_.typeEncode = @"block";
            NSString* key_ = [ASTUtil nodeKey:ASTNodeTypeBlockDeclarator keyword:nil];
            [_typeEncodeDict setObject:ast_ forKey:key_];
        }
    }
...
+ (ASTContext*)linkContextToRoot:(ASTNode*)rootNode
{
    ...
    ASTVariable* varCtx = [[ASTVariable alloc]init];
    varCtx.value = rootNode.context;
    varCtx.type  = ASTNodeTypeVariable;
    [rootNode.context pushValue:varCtx forKey:@"currentContext"];
...
static NSMutableDictionary * astKeywordMap = nil;

+ (void)registName:(NSString *)key val:(NSString *)val {
    if(!astKeywordMap) {
        astKeywordMap = [[NSMutableDictionary alloc] init];
    }
    [astKeywordMap setObject:val forKey:key];
}

+ (NSString*) convertKeyword : (NSString *) key {
    if(astKeywordMap) {
        NSString * val = [astKeywordMap objectForKey:key];
        if(val) {
            return val;
        }
    }
