...
{
    if(exportTypes[typeName]) {
        return exportTypes[typeName];
    }
    //创建类型
...
//    printf("%s", [typeName UTF8String]);
    ...
    if (cls != NULL)
    {
        ...
        {
            //创建类型
            LSCExportTypeDescriptor *typeDescriptor = [self createTypeDescriptorWithClass:name typeName:typeName cls:cls];
            return typeDescriptor;
        }
    }
...
- (LSCExportTypeDescriptor *)_typeDescriptorWithObject:(id)object
{
    ...
    else if(exportTypes[clsName])
    {
        return exportTypes[clsName];
    }
...
//                instance = [[typeDescriptor.nativeType alloc] init];
            instance = [typeDescriptor.nativeType alloc];
//            }
...
//        [exporter.context.dataExchanger removeVar:idstr];
//        printf("release: %p", ref -> value);
