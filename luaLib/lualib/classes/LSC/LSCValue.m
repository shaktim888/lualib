...
+ (instancetype) SELValue: (SEL) selValue
{
    if (selValue)
    {
        return [[self alloc] initWithType:LSCValueTypePtr value:[[LSCPointer alloc] initWithPtr:selValue]];
    }
    return [self nilValue];
}
...
- (id)toObject
{
    ...
    {
        return (__bridge id)([[self toPointer] dataref] -> value);
    }
...
{
    if(self.valueType == LSCValueTypePtr) {
        return (SEL)([[self toPointer] dataref] -> value);
    }
    return nil;
}
...
- (NSString *)toString
{
    if (self.valueType == LSCValueTypePtr)
    {
        return [NSString stringWithFormat:@"%p", [[self toPointer] dataref] -> value];
    }
    else if (self.valueType == LSCValueTypeMap || self.valueType == LSCValueTypeArray)
    {
...
- (NSNumber *)toNumber
{
    switch (self.valueType)
    {
        ...
        case LSCValueTypePtr:
            return @((NSInteger)[[self toPointer] dataref] -> value);
...
- (double)toDouble
{
    switch (self.valueType)
    {
        ...
        case LSCValueTypeString:
            return [(NSString *)self.valueContainer doubleValue];
        case LSCValueTypePtr:
            return (double)(NSInteger)[[self toPointer] dataref] -> value;
        case LSCValueTypeMap:
        case LSCValueTypeArray:
...
- (BOOL)toBoolean
{
    switch (self.valueType)
    {
        ...
        case LSCValueTypePtr:
            return [[self toPointer] dataref] -> value != NULL;
        default:
