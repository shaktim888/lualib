        ...
        if (arguments.count > startIndex)
        {
            ...
            for (int i = startIndex; i < arguments.count; i++)
            {
                ...
                switch (value.valueType)
                {
                    ...
                    default:
                        if ([[value toObject] isKindOfClass:[OCBlockWrapper class]])
                        {
                            [signArr addObject:@"@?"];
                            [signStrRegexp appendString:@"@\\?"];
                        } else {
...
- (LSCValue *) invokeMethod:(Class) cls caller: (id)caller invocation:(NSInvocation *)invocation arguments:(NSArray *)arguments
{
   ...
   for (int i = 2; i < argLength; i++)
   {
       ...
       LSCValue *value = nil;
       if (arguments && i - fixedIndex < arguments.count)
       {
       ...
       {
           switch (argType[0]) {
                   #define OCNumber_CASE(_typeString, _type) \
                   case _typeString: { \
                       _type arg = (_type)[value toDouble];  \
                       [invocation setArgument:&arg atIndex:i];  \
                       break;  \
                   }
                   OCNumber_CASE('c', char)
                   OCNumber_CASE('C', unsigned char)
                   OCNumber_CASE('s', short)
                   OCNumber_CASE('S', unsigned short)
                   OCNumber_CASE('i', int)
                   OCNumber_CASE('I', unsigned int)
                   OCNumber_CASE('l', long)
                   OCNumber_CASE('L', unsigned long)
                   OCNumber_CASE('q', long long)
                   OCNumber_CASE('Q', unsigned long long)
           }
       }
       else if (strcmp(argType, @encode(id)) == 0)
       {
           ...
           if (end != NSNotFound) {
               ...
               if (structDefine) {
                   ...
                   if (size) {
                       ...
                       [OCExtension getStructDataWidthDict:ret dict:[value toObject] structDefine:structDefine];
           ...
           if(value.valueType == LSCValueTypeNil) {
               void * ptr = nil;
               [invocation setArgument:&ptr atIndex:i];
           }
           else
           {
               OCBlockWrapper * wrap = [value toObject];
...
{
    return [self invokeMethod:self.nativeType caller:instance invocation:invocation arguments:arguments];
}
